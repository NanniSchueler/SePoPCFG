...
# For Linux
#process_trainer = subprocess.Popen(["python", "trainer.py", "-tdata/train.txt", "-c"+str(0.5), "-rdebug", "-mglove-twitter-200", "-l8", "-k0", "-n3"])
# For Windows
process_trainer = subprocess.Popen(["python", "trainer.py", "-tdata/train.txt", "-c"+str(0.5), "-rdebug", "-mglove-twitter-200", "-l8", "-k0", "-n3"], shell=True)
process_trainer.wait()
process_trainer.kill()
output_file = open("suggestion_list.txt", "w")
# For Linux
#process = subprocess.Popen(["python","pcfg_guesser.py","-ssession2","-rdebug","-n10000000", "-p4#1#1"], stdout=output_file, universal_newlines=True)
# For Windows
