...
process_trainer = subprocess.Popen(["python", "trainer.py", "-tdata/train.txt", "-c"+str(0.5), "-rcombined", "-mglove-twitter-200", "-l8", "-k425", "-n3"])
...
process = subprocess.Popen(["python", "pcfg_guesser.py", "-ssession2", "-rcombined", "-n55000000", "-p8#1#0"], stdout=output_file, universal_newlines=True)
process.wait()
process.kill()
# TODO: Test set Pfad anpassen
eval.main("test.txt", ["suggestion_list_pcfg_combined.txt"])
